<template>
  <admin-product-layout title="상품 등록">
    <form class="product-form" @submit.prevent="submit">
      <section><h2>기본 정보</h2><div class="grid">
        <label>상품 코드*<input v-model.trim="form.productCode" required></label><label>판매자*<input v-model.trim="form.sellerName" required></label>
        <label>상품명(한글)*<input v-model.trim="form.productNameKo" required></label><label>상품명(영문)<input v-model.trim="form.productNameEn"></label>
        <label>브랜드<input v-model.trim="form.brandName"></label><label>카테고리*<input v-model.trim="form.categoryName" required></label>
      ...
      <section><h2>판매·전시</h2><div class="grid">
        <label>상품 상태<select v-model="form.productStatus"><option v-for="status in statuses" :key="status.value" :value="status.value">{{ status.label }}</option></select></label>
      ...
      <p v-if="errorMessage" class="error" role="alert">{{ errorMessage }}</p><div class="actions"><router-link :to="{ name: 'adminProductList' }">취소</router-link><button :disabled="isSubmitting">{{ isSubmitting ? '등록 중...' : '상품 등록' }}</button></div>
    </form>
  </admin-product-layout>
</template>
<script>
import AdminProductLayout from '@/components/admin/AdminProductLayout.vue'
import { createAdminProduct } from '@/api/admin.js'
export default {
  name: 'AdminProductCreateView',
  components: { AdminProductLayout },
  data () { return { files: [], previews: [], isSubmitting: false, errorMessage: '', statuses: [{ value: 'READY', label: '판매대기' }, { value: 'ON_SALE', label: '판매중' }, { value: 'SOLD_OUT', label: '품절' }, { value: 'SALE_ENDED', label: '판매종료' }, { value: 'SALE_COMPLETED', label: '판매완료' }], form: { productCode: '', productNameKo: '', productNameEn: '', brandName: '', categoryName: '', sellerName: localStorage.getItem('adminName') || '', manufacturer: '', originCountry: '', costPrice: 0, salePrice: 0, discountPrice: null, stockQuantity: 0, safetyStockQuantity: 0, taxType: 'TAXABLE', shippingType: 'FREE', shippingFee: 0, productStatus: 'READY', useYn: 'Y', displayYn: 'Y', displayStartDate: null, displayEndDate: null, shortDescription: '', detailDescription: '', searchKeywords: '' } } },
